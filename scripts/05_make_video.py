...
def build_srt_file(timings):
    ...
    with open("output/subtitles.srt", "w", encoding="utf-8") as f:
        for section in timings:
            ...
            # Phrase-by-phrase display — show 1-2 short lines at a time
            # This creates visual pacing that keeps eyes locked on screen
            wrapped_lines = textwrap.wrap(text, width=25)

            if not wrapped_lines:
                wrapped_lines = [text[:25]]

            # Group into blocks of up to 2 lines (phrase-by-phrase, not paragraphs)
            blocks = []
...
def assemble_video(voiceover_path, voiceover_duration, timings=None):
    ...
    if srt_path and os.path.exists("output/subtitles.srt"):
        ...
        print(f"  Burning YouTube subtitles (centered, phrase-by-phrase)...")
